...
const handleSubmit = async (e) => {
  ...
  try {
      ...
      const fetchbooks = async () => {
        try {
          ...
          if (Array.isArray(response.data)) {
              setBooks(response.data);
              setTotalBooks(response.data.length);
              setbooktitles(response.data.map(book => book.title))
              console.log("all title lst", response.data.map(book => book.title))
              // console.log("here o oooooo ooo",booktitles)
          } else {
            console.error('Invalid data format:', response.data);
          }
        } catch (error) {
    ...
    const handleBookSelection = (event) => {
      ...
      // Update form fields with the details of the selected book
      setTitle(book.title);
  ...
  <form className="form_container" onSubmit={handleSubmit}>
    ...
    <input
      ...
      style={{width:'340px'}}
    />
    {/* Input field for selecting book */}
    <select id="bookList" onChange={handleBookSelection} className="input">
    ...
    {/* Additional input fields for book details */}
    {/* <input
      type="text"
      placeholder="Author"
      name="author"
    ...
    <button type="submit" className="btn" style={{position:'relative',top:'30px',width:'90px',fontSize:'17px',height:'40px',border:'none',borderRadius:'5px'}}>Add Book</button>
  </form>
</div>
